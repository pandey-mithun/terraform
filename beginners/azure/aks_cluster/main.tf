...
data "azurerm_resource_group" "k8terraform" {
...
#   resource_group_name = data.azurerm_resource_group.k8terraform.name
...
resource "azurerm_kubernetes_cluster" "k8cluster" {
  ...
  location            = data.azurerm_resource_group.k8terraform.location
  resource_group_name = data.azurerm_resource_group.k8terraform.name
  ...
  # Getting error Blocks of type "addon_profile" are not expected here.
  # addon_profile {
  #   oms_agent {
  #     enabled                    = true
